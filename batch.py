...
class Batch(ctypes.Structure):
    _fields_ = [
        ('num_active_features_white_stm', ctypes.c_uint32),
        ('active_features_white_stm', ctypes.POINTER(ctypes.c_int16)),
        ('num_active_features_black_stm', ctypes.c_uint32),
    ...
    def features_white_stm_dense_tensor(self):
        features_tensor = torch.zeros(BATCH_SIZE, 768, device=device)

        arr = np.ctypeslib.as_array(self.active_features_white_stm, shape=(self.num_active_features_white_stm, 2))
    ...
    def features_black_stm_dense_tensor(self):
        features_tensor = torch.zeros(BATCH_SIZE, 768, device=device)

        arr = np.ctypeslib.as_array(self.active_features_black_stm, shape=(self.num_active_features_black_stm, 2))
        indices_tensor = torch.from_numpy(arr).int()
